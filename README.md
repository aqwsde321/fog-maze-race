...
- 아이템 맵과 `ice_trap` 1차 규칙 초안: [docs/item-maps.md](./docs/item-maps.md)
...
현재 아이템전은 방 안 대기 화면에서 방장이 `경기`를 `일반전 / 아이템전`으로 전환해서 켭니다.

- `일반전`은 모든 맵에서 아이템 박스가 생성되지 않음
- `아이템전`은 모든 맵에서 아이템 박스가 생성될 수 있음
- 인벤토리 `1칸`
- 아이템 보유 중 추가 박스 획득 불가
- 기본 생성 규칙은 `레이서 수 x 2`
- 관리자 맵 편집에서는 `참가자 수 배수` 또는 `고정 개수`로 생성 수만 조절 가능
- 생성 위치는 `mazeZone` 안 길 타일 랜덤 스폰
- 아이템 박스는 `ice_trap`, `return_trap`, `flare`, `boost`, `scanner` 중 하나를 랜덤 제공
- `Space`로 사용
- `ice_trap`: 상대가 밟으면 `1500ms` 빙결
- `return_trap`: 상대가 밟으면 공용 시작 첫 슬롯으로 복귀
- `flare`: `4000ms` 동안 시야 반경 `+1`
- `boost`: `3000ms` 동안 입력 1회당 최대 `2칸` 이동
- `scanner`: `4000ms` 동안 자기 캐릭터 근처에 목표 방향 힌트 제공
