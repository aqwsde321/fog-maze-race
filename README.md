...
- 탐험형 봇은 방장 UI에서 봇마다 전략을 따로 고를 수 있고, 현재는 `frontier`, `tremaux` 두 전략을 지원함
...
- `frontier`는 현재 시야 기준의 미지 경계(frontier)를 넓게 찾아가는 기본 전략이고, `tremaux`는 이미 지난 통로를 더 강하게 피해서 반복을 줄이는 전략임
...
4. 탐험형을 선택했다면 각 봇 행마다 `frontier`, `tremaux` 전략을 따로 고를 수 있습니다.
5. `bot_race`에서는 사람은 관전 상태로 남고, 우측 `레이서` 목록에는 실제 봇 레이서만 표시됩니다.
