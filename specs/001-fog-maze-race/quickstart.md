...
- `results-overlay-15players.spec.ts`: 다인 결과 화면 스크롤과 순위 표시
...
- 방장만 시작, 이름 변경, 강제 종료를 수행할 수 있고 비방장에게는 시작 버튼이 보이지 않는다.
- 방장 UI에서 봇 이름을 수정해 봇을 추가/제거할 수 있고, 탐험형은 `frontier`, `tremaux` 전략을 고를 수 있다.
- `봇 전용 방`에서는 사람은 관전 상태로 남고, `레이서` 목록에는 실제 봇만 표시된다.
- `봇 전용 방`의 사람 관전자는 채팅 가능하고 레이서 슬롯을 차지하지 않는다.
- `waiting` 과 `countdown` 동안에는 시작 구역 안에서만 이동할 수 있다.
- 서버 authoritative 이동이 벽을 막고 위치를 브로드캐스트한다.
- 기본 안개 시야는 `5x5`이고, 시야 밖 미로 플레이어를 가리며, 이미 본 미로 타일은 더 어두운 기억 타일로 남는다.
- 완주자는 전체 맵을 보고, 탐험형 봇은 goal을 처음부터 모른 채 현재 시야에 들어온 타일만 기억한다.
- 유예 시간 안에 재접속하면 플레이어 상태와 위치가 복구된다.
- 나가기 또는 유예 초과는 경기 종료 계산에서 제외된다.
- 결과 화면은 완주 순위 또는 `나감`을 보여주고 남은 초를 표시한 뒤 방을 `waiting`으로 되돌린다.
- 플레이어 마커는 서버가 배정한 색상과 모양을 사용하며, 현재 플레이어만 흰색 테두리로 강조된다.
