...
**저장 방식**: 서버 메모리 기반 방/매치 상태, 기본 맵 정의 + `data/maps.json` 기반 런타임 맵 레지스트리, 닉네임과 `playerId`용 `localStorage`
...
**제약 사항**: MVP는 단일 백엔드 인스턴스만 사용, 인스턴스 간 상태 복제 없음, 클라이언트는 방향만 전송, 아이템/리플레이/고급 커스터마이징 제외, 방 입장은 `waiting` 상태에서만 허용
