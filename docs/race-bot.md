...
방 안의 `봇 참가시키기` 패널에서는 이름 입력 행마다 전략을 따로 선택할 수 있습니다. 예를 들어 `bot1=frontier`, `bot2=tremaux`처럼 같은 방 안에서 서로 다른 탐험형 전략을 섞어 테스트할 수 있습니다.
...
4. 탐험형을 선택했다면 각 이름 입력 행마다 `frontier`, `tremaux` 전략을 따로 고릅니다.
...
- 탐험형을 선택하면 각 이름 입력 행마다 `frontier`, `tremaux` 전략을 따로 고를 수 있습니다.
- 탐험형일 때는 패널 안 `전략 설명` 툴팁으로 `frontier`, `tremaux` 차이를 바로 확인할 수 있습니다.
- 입력칸은 최대 5자까지 사용합니다.
