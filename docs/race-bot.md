...
- `bot_race` 방장은 `waiting` 상태에서 봇 배속을 `x1`부터 `x6`까지 설정할 수 있고, 현재 `join`/`explore` 봇의 기본 간격은 모두 `180ms`입니다.
...
- `tremaux`: `frontier` 기반을 유지하되, 이미 지나간 통로(edge)를 더 강하게 기록하고 다시 밟는 경로에 큰 패널티를 줍니다. 최근에는 동일 점수 갈림길에서 seed별 방향 우선순위와 동률 깨기를 추가해, 같은 시작점에서도 경로가 한쪽으로만 몰리지 않게 했습니다.
...
7. 각 frontier 후보까지의 경로를 BFS로 구하고, 전략에 맞는 점수식을 적용한 뒤 좋은 후보군 안에서 seed별로 하나를 고릅니다. `frontier`는 근접 후보 분산을 더 강하게 섞고, `tremaux`도 완전 동률 후보에서는 seed를 사용해 갈림길을 분산합니다.
...
- 탐험형 seed는 닉네임 고정값이 아니라 매 경기 다시 뽑고, 같은 경기 안에서는 중복 없이 균등하게 퍼진 seed 묶음을 나눠 받아서 번호 편향과 한 판 안의 완전 군집을 함께 줄입니다.
...
4. `bot_race` 방이라면 같은 패널에서 배속을 `x1`부터 `x6`까지 고를 수 있습니다.
5. 탐험형을 선택했다면 각 이름 입력 행마다 `frontier`, `tremaux`, `wall` 전략을 따로 고릅니다.
6. 패널 아래 `봇 참가시키기` 버튼으로 추가합니다.
7. 이미 들어온 봇은 같은 패널의 `현재 봇` 목록에서 개별 제거하거나 `모두 제거`할 수 있습니다.
8. 시작과 재시작은 계속 방장이 UI에서 직접 합니다.

UI 동작 메모:

- 기본적으로 `bot1`, `bot2`처럼 이름이 미리 채워집니다.
- `bot_race`에서는 `waiting` 상태일 때만 배속을 `x1`부터 `x6`까지 바꿀 수 있습니다.
