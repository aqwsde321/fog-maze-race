...
탐험형 봇은 전체 맵을 바로 풀지 않고, 실제로 본 타일만 기억하면서 움직입니다. 빠른 봇이 "전체 맵 최단 경로"라면, 탐험형 봇은 "부분 관측 + 기억 + frontier 탐색"에 가깝습니다.
...
7. 각 frontier 후보까지의 경로를 BFS로 구하고, 점수가 가장 좋은 후보군 안에서 seed별로 하나를 고릅니다.

frontier 점수는 아래 요소를 합쳐 계산합니다.
